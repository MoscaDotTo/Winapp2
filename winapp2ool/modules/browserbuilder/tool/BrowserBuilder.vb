...
Public Module BrowserBuilder
    ...
    Private Sub processBrowserBuilder(chromiumIni As iniFile2,
        ...
        Dim flavorAdd = loadFlavorFile(sourceDir, "browser_additions.ini")
        Dim flavorSecRem = loadFlavorFile(sourceDir, "browser_section_removals.ini")
        Dim flavorNameRem = loadFlavorFile(sourceDir, "browser_name_removals.ini")
        Dim flavorValRem = loadFlavorFile(sourceDir, "browser_value_removals.ini")
        Dim flavorSecRep = loadFlavorFile(sourceDir, "browser_section_replacements.ini")
        Dim flavorKeyRep = loadFlavorFile(sourceDir, "browser_key_replacements.ini")

        Flavorize(outputFile, outputFile, output, flavorAdd, flavorSecRem, flavorNameRem, flavorValRem, flavorSecRep, flavorKeyRep)

        Dim legacyForDebug = IniFileBridge.ToIniFile(outputFile)
        legacyForDebug.Sections = remotedebug(legacyForDebug, True).Sections
        outputFile = IniFileBridge.ToIniFile2(legacyForDebug)

        Dim leadingComments As New List(Of String) From {
            $"; Version {DateTime.Now.ToString("yyMMdd")}",
            $"; # of entries: {legacyForDebug.Sections.Count:#,###}",
            $"; {legacyForDebug.Name} is generated by the Winapp2ool Browser Builder",
        ...
        sb.AppendLine()
        sb.Append(legacyForDebug.toString)

        legacyForDebug.overwriteToFile(sb.ToString())
