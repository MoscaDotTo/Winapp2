...
''' <summary>
''' Renders Diff results as <c>MenuSection</c> output and summary statistics.
''' </summary>
Public Class DiffOutputRenderer
    ...
    Public Function LogPostDiff() As MenuSection
        ...
        Dim netDiff = _file2.Sections.Count - _file1.Sections.Count
    ...
    Public Function SummarizeMergers() As List(Of MenuSection)
        ...
        For Each oldEntry In _state.MergedEntries.OldToNewMergeDict
            ...
            result = If(newTargets.Count = 1,
                          MakeDiff(_file1.Sections(oldName), 4, _file2.Sections(newTargets(0))),
                          MakeDiffMultiTarget(_file1.Sections(oldName), newTargets))
    ...
    Public Function SummarizeRenames() As List(Of MenuSection)
        ...
        For Each entry In _state.MergedEntries.RenamedEntryNames
            ...
            out.Add(MakeDiff(_file1.Sections(oldName), 3, _file2.Sections(entry)))
    ...
    Public Function ItemizeMergers() As List(Of MenuSection)
        ...
        For Each oldEntry In _state.MergedEntries.OldToNewMergeDict
            ...
            If _file1.Sections.ContainsKey(targetEntry) Then
                _state.MergedEntries.MergeDict(targetEntry).Add(targetEntry)
            End If

            For i = 0 To _state.MergedEntries.MergeDict(targetEntry).Count - 1

                Dim oldEnt = _file1.Sections(_state.MergedEntries.MergeDict(targetEntry)(i))

                For Each key In oldEnt.Keys.Keys
            ...
            _keyAnalyzer.FindModifications(mergedOldEntries, _file2.Sections(targetEntry))
    ...
    Public Function ItemizeAdditions() As List(Of MenuSection)
        ...
        For Each entry In _state.ModifiedEntries.AddedEntryNames.ToList
            ...
            Dim section = _file2.Sections(entry)
    ...
    Public Function ItemizeModifications(Optional isMerger As Boolean = False) As List(Of MenuSection)
        ...
        For Each entry In _state.ModifiedEntries.ModifiedEntryNames.ToList
            ...
            Dim newSectionVer = _file2.Sections(entry)
    ...
    ''' This is a distinct category from both "modified" and "plain added" entries.
    ''' </summary>
    Public Function ItemizeAddedEntriesWithMergers() As List(Of MenuSection)
        ...
        For Each entry In _state.ModifiedEntries.AddedEntryNames.ToList
            ...
            ' Build combined old entry and compute modifications
            Dim uniqueKeyValues = New HashSet(Of String)(StringComparer.OrdinalIgnoreCase)
            ...
            For i = 0 To _state.MergedEntries.MergeDict(entry).Count - 1
                ...
                If Not _file1.Sections.ContainsKey(oldEntryName) Then Continue For

                Dim oldEnt = _file1.Sections(oldEntryName)
            ...
            Dim mergedOldEntries = New iniSection(entryBuilder)
            _keyAnalyzer.FindModificationsForAddedEntry(mergedOldEntries, _file2.Sections(entry))
            ...
            For Each key In _file2.Sections(entry).Keys.Keys

                If addedKeys.Keys.Contains(key) OrElse removedKeys.Keys.Contains(key) Then Continue For

                ' Skip if it's in updated keys (modified)
                Dim isUpdated = False
