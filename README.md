...
-  [CCleaner](https://www.ccleaner.com/ccleaner):
   -  Download the latest winapp2.ini from this repo and place it in the same folder as ccleaner.exe
 ...
 * DetectOS criteria supercede all other detection criteria. 
...
* If the target of these keys exist, the entry will be shown to the user. Otherwise it wont. 
...
`ExcludeKey`
* Used to exclude a file, folder, or registry path from cleaning, even if otherwise included in a FileKey or RegKey
...
`%CommonProgramFiles%`* | `C:\Program Files\Common Files`|N/A
